...
function Administrador() {
    ...
    useEffect(() => {
        const obtenerUsuarios = async () => {
            setLoading(true);
            try {
                const response = await axios.get('https://audiosmadres.onrender.com/get-all-users');
    ...
    const filtrarUsuarios = async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.get(`https://audiosmadres.onrender.com/filter-users?field=${filtro}&value=${textoFiltro}`);
            setUsuarios(response.data.users);
        } catch (error) {
    ...
    const descargarAudio = async (audioRef) => {
        ...
        try {
            const response = await axios.get(`https://audiosmadres.onrender.com/download-audio?ref=${ref}`, { responseType: 'blob' });
    ...
    const descargarCSV = async () => {
        try {
            const response = await axios.get('https://audiosmadres.onrender.com/export-users-csv', {
                responseType: 'blob'
            });
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
    ...
    return (
        <div className='w-full h-screen flex flex-col justify-center items-center'>
            ...
            <div className='bg-slate-400 py-4 px-4'>
                ...
                <button onClick={descargarCSV} className='bg-blue-500 text-white p-2 rounded-lg'>Descargar CSV</button>
                <ul className='mr-5 mb-5'>
                    {usuarios.map(usuario => (
                        <li key={usuario.id} className='mr-5'>
                            {usuario.nombre} {usuario.apellido} - {usuario.tipodocumento} {usuario.numerodocumento} - {usuario.email}
                            <button onClick={() => descargarAudio(usuario.audioRef)} className='mr-5 mb-5 ml-5 bg-red-500 rounded-lg px-4'>Descargar Audio</button>
                        </li>
                    ))}
                </ul>
            </div>
